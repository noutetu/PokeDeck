...
public class DeckView : MonoBehaviour
{
    ...
    {
        // サンプルデッキかどうかを判定
        isReadOnly = DeckManager.Instance.IsCurrentDeckSample();
    ...
    private void CreateCardItem(string cardId)
    {
        ...
        if (cardModel != null)
        {
            // カード画像を設定（ImageCacheManagerを使用してキャッシュを確認）
            RawImage cardImage = cardItem.GetComponentInChildren<RawImage>();
            if (cardImage != null)
            {
                // キャッシュから画像を取得を試行
                if (ImageCacheManager.Instance != null && ImageCacheManager.Instance.IsCardTextureCached(cardModel))
                {
                    Texture2D cachedTexture = ImageCacheManager.Instance.GetCachedCardTexture(cardModel);
                    cardImage.texture = cachedTexture;
                    cardModel.imageTexture = cachedTexture;
                }
                else if (cardModel.imageTexture != null)
                {
                    // 既存のテクスチャがある場合はそれを使用
    ...
    // ----------------------------------------------------------------------
    // カードアイテムを非同期で作成（画像キャッシュ検証付き）
    // ----------------------------------------------------------------------
    private async Task CreateCardItemAsync(string cardId)
    {
        // 必要なプレハブやコンテナが設定されていない場合は処理を中断
        if (cardItemPrefab == null || cardContainer == null)
            return;
            ...
            if (cardImage != null)
            {
                ...
                if (ImageCacheManager.Instance != null)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            Texture2D texture = await ImageCacheManager.Instance.GetCardTextureAsync(cardModel);
                        ...
                        {
                            Debug.LogError($"デッキカード画像の読み込みでエラー: {ex.Message}");
                            // エラー時はデフォルトテクスチャを設定
                            if (cardImage != null)
                            {
                                cardImage.texture = ImageCacheManager.Instance.GetDefaultTexture();
    ...
    {
        if (deckCountText != null && currentDeck != null)
        {
