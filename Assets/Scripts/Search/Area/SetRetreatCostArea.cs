...
public class SetRetreatCostArea : MonoBehaviour, IFilterArea
{
    ...
    private void OnDropdownValueChanged(int index)
    {
        ...
        {
            bool valueChanged = selectedRetreatCost != cost;
            ...
            // 値が変更された場合はフィルター変更を通知
            if (valueChanged || selectedComparisonType != RetreatComparisonType.None)
            {
                OnFilterChanged?.Invoke();
            }
        }
    ...
    private void OnToggleValueChanged(bool isOn, RetreatComparisonType comparisonType)
    {
        if (isOn)
        {
            ...
            // トグルの視覚的な状態を更新
            UpdateToggleVisualState(comparisonType, true);
        ...
        else if (selectedComparisonType == comparisonType)
        {
            ...
            // トグルの視覚的な状態を更新
            UpdateToggleVisualState(comparisonType, false);
    ...
    // ----------------------------------------------------------------------
    // トグルの視覚的な状態を更新するヘルパーメソッド
    // ----------------------------------------------------------------------
    private void UpdateToggleVisualState(RetreatComparisonType comparisonType, bool isOn)
    {
        ...
        {
            case RetreatComparisonType.LessOrEqual:
                targetToggle = lessOrEqualToggle;
                break;
            case RetreatComparisonType.Equal:
                targetToggle = equalToggle;
                break;
            case RetreatComparisonType.GreaterOrEqual:
                targetToggle = greaterOrEqualToggle;
                break;
        }
        ...
        {
            // SimpleToggleColorコンポーネントを更新
            SimpleToggleColor colorComponent = targetToggle.GetComponent<SimpleToggleColor>();
            if (colorComponent != null)
            {
            ...
            // TrueShadowToggleInsetコンポーネントを更新
            TrueShadowToggleInset shadowComponent = targetToggle.GetComponent<TrueShadowToggleInset>();
            if (shadowComponent != null)
            {
